...
class Template(Widget):
    ...
    def __init__(self, parent, opts=None, **options):
        ...
        with open(os.path.join(locations.pwt_loc, 'template', 'template.css')) as fi:
    ...
    def _handle_set(self, op):
        ...
        for key, value in op.args.items():
            if key == 'svg':
                downloadsvg(op.args['svg'], self.width.value, self.height.value, os.path.join(locations.pwt_loc, 'template', 'template.css'), name=__class__.__name__)
            if key == 'pdf':
                downloadpdf(op.args['pdf'], self.width.value, self.height.value, os.path.join(locations.pwt_loc, 'template', 'template.css'), name=__class__.__name__)
