...
class Audio(Widget):
    ...
    _rwt_class_name = 'pwt.customs.Audio'
    _defstyle_ = BitField(Widget._defstyle_)

    @constructor('Audio')
    def __init__(self, parent, **options):
        Widget.__init__(self, parent, **options)
        self.theme = AudioTheme(self, session.runtime.mngr.theme)
        self._sources = []
    ...
    def addsrc(self, src):
        ...
        with open(os.path.abspath(src['source']), "rb") as f:
            resource = session.runtime.mngr.resources.registerf(os.path.basename(src.get('source')), src.get('type', 'audio/mpeg'), f, encode=False)
            session.runtime << RWTCallOperation(self.id, 'addSrc', {'source': resource.location, 'type': src['type']})


class AudioTheme(WidgetTheme):

    def __init__(self, widget, theme):
        WidgetTheme.__init__(self, widget, theme, 'Audio')

    @property
    def borders(self):
        return [self._theme.get_property('border-%s' % b, 'Audio', self.styles(), self.states()) for b in ('top', 'right', 'bottom', 'left')]

    @property
    def bg(self):
        if self._bg: return self._bg
        return self._theme.get_property('background-color', 'Audio', self.styles(), self.states())
    ...
    @property
    def padding(self):
        return self._theme.get_property('padding', 'Audio', self.styles(), self.states())

    @property
    def font(self):
        return self._theme.get_property('font', 'Audio', self.styles(), self.states())

    @property
    def margin(self):
        return self._theme.get_property('margin', 'Audio', self.custom_variant(), self.styles(), self.states())
