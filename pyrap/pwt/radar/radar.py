    ...
    def _handle_set(self, op):
        ...
        for key, value in op.args.items():
            if key == 'svg':
                ...
                downloadsvg(op.args['svg'][0], self.width.value, self.height.value, os.path.join(locations.pwt_loc, 'radar', 'radar.css'), name=fname)
            if key == 'pdf':
                fname = op.args['pdf'][1]
                if fname is None:
                    fname = __class__.__name__
                downloadpdf(op.args['pdf'][0], self.width.value, self.height.value, os.path.join(locations.pwt_loc, 'radar', 'radar.css'), name=fname)
