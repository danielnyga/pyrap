...
pwt_bubblyclusters.BubblyClusters = function( parent, opts) {
...
pwt_bubblyclusters.BubblyClusters.prototype = {
    ...
    setData : function ( data ) {
        this._distinctclusters = this.distinct(data);
        this._clusters = new Array(this._distinctclusters.length);
        this._data = data;
        this._cwidth = 200;//this._w > 0 ? this._w : 960;
        this._cheight = 200;//this._h > 0 ? this._h : 600;
        var that = this;
        ...
        this._nodes = d3.range(this._data.length).map(function(i) {
             ...
             d = {
                 ...
                 x: Math.cos(i / that._distinctclusters.length * 2 * Math.PI) * 200 + that._cwidth / 2 + Math.random(),
                 y: Math.sin(i / that._distinctclusters.length * 2 * Math.PI) * 200 + that._cheight / 2 + Math.random()
             };
             if (!that._clusters[i] || (that._data[i].radius > that._clusters[i].radius)) that._clusters[i] = d;
             return d;
         });
        this.update();
    },
    ...
    collide : function(alpha) {
        var quadtree = d3.geom.quadtree(this._nodes);
        return function(d) {
            var r = d.radius + this._maxRadius + Math.max(this._padding, this._clusterPadding),
            nx1 = d.x - r,
            nx2 = d.x + r,
            ny1 = d.y - r,
            ny2 = d.y + r;
            quadtree.visit(function(quad, x1, y1, x2, y2) {
                if (quad.point && (quad.point !== d)) {
                    ...
                    y = d.y - quad.point.y,
                    l = Math.sqrt(x * x + y * y),
                    r = d.radius + quad.point.radius + (d.cluster === quad.point.cluster ? this._padding : this._clusterPadding);
                    if (l < r) {
                        l = (l - r) / l * alpha;
                        d.x -= x *= l;
                        d.y -= y *= l;
